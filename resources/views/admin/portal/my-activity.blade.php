...
@section('content')
    ...
    @php
        ...
        $hasFilters = request()->filled('company_id') || request()->filled('job_id') || $dateFilter->isActive();
    @endphp

    <div class="portal-page">
        @include('partials.portal-nav', ['active' => 'my-activity'])
        ...
        <div class="portal-hero d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div>
                ...
                <h1 class="portal-hero-title">My activity</h1>
                <p class="portal-hero-sub">Candidates you applied on behalf for your assigned companies.</p>
            </div>
            <span class="badge rounded-pill bg-light text-dark px-3 py-2 fw-semibold">
                <i class="bi bi-send-check me-1"></i>{{ number_format($applications->total()) }} applies
            </span>
        </div>
        ...
        <div class="portal-filters-card">
            <div class="portal-filters-head">
                ...
                @if($hasFilters)
                    <a href="{{ route('admin.portal.my-activity') }}" class="btn btn-sm btn-link text-decoration-none">Reset all</a>
                @endif
            </div>
            <form method="GET" action="{{ route('admin.portal.my-activity') }}" class="portal-filters-body">
