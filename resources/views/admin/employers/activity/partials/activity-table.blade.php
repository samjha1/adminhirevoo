...
<div class="company-activity-table-wrap">
    <div class="table-responsive">
        <table class="table table-hover align-middle company-activity-table mb-0">
            <thead>
                <tr>
                    <th>When</th>
                    @if($showStaffColumn)
            ...
            <tbody>
                @forelse($activities as $item)
                    ...
                    <tr>
                        ...
                        <td>{{ \Illuminate\Support\Str::limit($item['company_name'], 40) }}</td>
                        <td class="text-end">
                            @if($item['url'])
                                <a href="{{ $item['url'] }}" class="btn btn-sm btn-outline-success">View</a>
                            @else
